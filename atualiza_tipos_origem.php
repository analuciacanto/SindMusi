...
if( isset( $_POST['tiposRegistros'], $_POST['origensRegistros'] ) ){
	...
	foreach( $tiposRegistros as $t ){
		$t = trim($t);
		if( $t != "" ){//Pego todos os elementos não vazios da lista
			$tiposReg[] = $t;
		}
	}
	$tiposReg_json = json_encode( $tiposReg );
	if( !file_put_contents( "tipos-de-registros.json", $tiposReg_json ) ){
